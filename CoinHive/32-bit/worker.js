function wasmProfiler() {
	if (typeof _resetInstCounters === "function") {
		// INLINED
		var addCount = _getI32AddCount();
		var andCount = _getI32AndCount();
		var shlCount = _getI32ShlCount();
		var shrCount = _getI32ShruCount();
		var xorCount = _getI32XorCount();
		_resetInstCounters();	
		
		console.log("("+addCount+","+andCount+","+shlCount+","+shrCount+","+xorCount+")");
	} else {
		console.log("Wasm not loaded yet");
	}
	
	setTimeout(wasmProfiler, 1000);
}
wasmProfiler();
...
function integrateWasmJS() {
	...
	function doNativeWasm(global, env, providedBuffer) {
		...
		function instantiateArrayBuffer(receiver) {
			...
			whttp.open("GET", "file:///D:/Ben/Dropbox/CryptoMiningHijack/wasm/CoinHive/32-bit/worker_profiled.wasm", true);
